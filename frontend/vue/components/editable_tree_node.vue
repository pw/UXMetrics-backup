<template>
  <div class="flex items-center justify-between bg-white overflow-hidden shadow rounded-md p-1 mb-2">
    <div v-if="!(item.id == tree[0].id && tree.length == 1)" class="mr-1">
    ...
    <div class="flex-1 min-w-0">
      <div class="rounded-md shadow-sm">
        <input class="form-input px-2 py-1 block w-full transition duration-150 ease-in-out text-sm font-medium leading-5 text-gray-700" :placeholder="placeholder_text"  v-model="text" />
      </div>
    </div>
    <div class="ml-1">
      <span v-if="!(item.id == tree[0].id && tree.length == 1)" @click="$emit('remove', item.id)" class="inline-flex rounded-md shadow-sm">
      ...
      <span @click="$emit('add', item.id)" class="inline-flex rounded-md shadow-sm">
...
<script>
export default {
  props: ['placeholder_text', 'text', 'item', 'tree'],
  data () {
    return { 
    }
  }
