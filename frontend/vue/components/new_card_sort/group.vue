  ...
  <div class="flex items-top justify-between bg-white overflow-hidden shadow rounded-md p-2 mb-3">
    ...
    <div class="flex-1 min-w-0">
      <div class="rounded-md shadow-sm">
        <input ref="input" class="form-input px-2 py-1 block w-full transition duration-150 ease-in-out text-sm font-medium leading-5 text-gray-700" @input="$emit('input', $event.target.value)" @blur="$emit('updateGroup', group_id)" @keyup.enter="$emit('nextGroup', group_id)" :value="value" placeholder="Group Name" />
      </div>
    </div>
    <div class="flex ml-2">
