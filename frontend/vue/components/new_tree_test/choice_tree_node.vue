<template>
  <div>
    <div v-if="element.children.length == 0" class="flex items-center justify-between bg-white overflow-hidden shadow rounded-md h-8 px-2 py-1 mb-2 cursor-pointer" :style="{'margin-left': margin + 'rem'}">
      <div class="flex items-center">
        <input type="checkbox" class="form-checkbox h-4 w-4 text-purple-600 transition duration-150 ease-in-out element" :id="element.id" :name="element.id" :checked="isSelected(element.id)" @click="$emit('toggleCorrectChoice', $event.target.id, lineage, element.text)"/> 
