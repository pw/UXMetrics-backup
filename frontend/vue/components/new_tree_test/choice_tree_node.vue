<template>
  <div>
    <div v-if="element.children.length == 0" class="flex items-center justify-between bg-white overflow-hidden shadow rounded-md h-8 px-2 py-1 mb-2" :class="'ml-' + margin">
    ...
    <div v-else class="flex items-center justify-between bg-gray-100 overflow-hidden rounded-md h-8 px-2 py-1 mb-2" :class="'ml-' + margin">
      <span class="ml-2 block text-sm leading-5 text-gray-700">{{ element.text }}</span>
    </div>
    <Node v-for="child in element.children" :element="child" :key="child.id" :lineage="lineage.concat([element.text])" :margin="margin + 6" @toggleCorrectChoice="toggleCorrectChoice" :correct_choice="correct_choice" />
  </div>	
</template>
